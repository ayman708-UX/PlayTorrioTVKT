...
/**
 * Pure-Kotlin VidUp Stream Scraper for PlayTorrioHTTP.
 * Ported 1-to-1 from Vyla VidUp provider.
 * Resolves movies and TV shows via vidup.to and enc-dec.app decryption pipeline.
 */
class VidUpScraper : StreamScraper {
    override val name: String = "PlayTorrioHTTP"

    companion object {
        private const val TAG = "VidUpScraper"
        private const val DOMAIN = "https://vidup.to"
    ...
    override suspend fun scrape(request: ScraperMediaRequest): List<ScraperStreamResult> = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val encReq = Request.Builder()
                .url("$API_BASE/enc-vidup?text=${URLEncoder.encode(rawToken, "UTF-8")}")
                .header("User-Agent", UA)
            ...
            Log.d(TAG, "Error scraping VidUp: ${e.message}")
            emptyList()
        }
    }
}
