package com.playtorrio.tv.core.server
...
import com.playtorrio.tv.core.streams.StreamBadgeRulesParser
...
class StreamBadgeConfigServer(
    ...
    private fun handleBadgeImport(session: IHTTPSession): Response {
        ...
        if (rawSourceUrl.isBlank() && pastedPayload.isBlank()) {
            return errorResponse(context?.getString(com.playtorrio.tv.R.string.web_stream_badge_error_url_required) ?: "Enter a badge JSON URL.")
        }
        if (pastedPayload.isBlank() &&
            !rawSourceUrl.startsWith("https://", ignoreCase = true) &&
            !rawSourceUrl.startsWith("http://", ignoreCase = true)
        ) {
            return errorResponse(context?.getString(com.playtorrio.tv.R.string.web_stream_badge_error_url_scheme) ?: "Badge URL must start with http:// or https://.")
        }
        ...
        if (!isExistingImport && currentRules.imports.size >= STREAM_BADGE_IMPORT_LIMIT) {
            return errorResponse(context?.getString(com.playtorrio.tv.R.string.web_stream_badge_error_import_limit, STREAM_BADGE_IMPORT_LIMIT) ?: "You can import up to $STREAM_BADGE_IMPORT_LIMIT badge URLs.")
        }
    ...
    private fun badgeImportFailedMessage(): String =
        context?.getString(com.playtorrio.tv.R.string.web_stream_badge_import_error) ?: "Badge import failed."
