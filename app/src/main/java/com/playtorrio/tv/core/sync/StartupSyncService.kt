package com.playtorrio.tv.core.sync
...
import com.playtorrio.tv.data.local.StartupSyncPreferences
...
class StartupSyncService @Inject constructor(
    ...
    private val cwEnrichmentCache: com.playtorrio.tv.data.local.ContinueWatchingEnrichmentCache
) {
    ...
    fun requestRealtimeSurfacePull(profileId: Int, surface: String) {
        ...
        scope.launch {
            ...
            when (surface) {
                ...
                "library" -> pullPlayTorrioLibrary(profileId)
                "watch_progress" -> {
    ...
    private suspend fun pullPeriodicLibrary(): Boolean {
        ...
        return pullPlayTorrioLibrary(profileId)
    }
    ...
    private suspend fun pullPlayTorrioLibrary(profileId: Int): Boolean {
        val isTrackingLibrary = libraryRepository.sourceMode.first() != LibrarySourceMode.LOCAL
        if (isTrackingLibrary) {
            libraryRepository.hasCompletedInitialPull = true
            Log.d(TAG, "Skipping PlayTorrio library pull for profile $profileId because a tracking library provider is active")
            return true
        }
        ...
        return try {
            ...
            libraryRepository.hasCompletedInitialPull = true
            Log.e(TAG, "Periodic PlayTorrio library pull failed profile=$profileId", e)
            false
        } finally {
            libraryRepository.isSyncingFromRemote = false
        }
