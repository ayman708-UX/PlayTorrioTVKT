...
class TorrServerBinary @Inject constructor(
    ...
    suspend fun start() = withContext(Dispatchers.IO) {
        ...
        if (!isBinaryAvailable) {
            throw TorrentException(context.getString(com.playtorrio.tv.R.string.torrent_error_binary_missing, binaryFile.absolutePath))
        }
        ...
        while (System.currentTimeMillis() < deadline) {
            ...
            if (!isProcessAlive(process)) {
                ...
                process = null
                throw TorrentException(context.getString(com.playtorrio.tv.R.string.torrent_error_process_died, exitCode))
            }
            delay(HEALTH_CHECK_INTERVAL_MS)
        }

        stop()
        throw TorrentException(context.getString(com.playtorrio.tv.R.string.torrent_error_start_timeout, (STARTUP_TIMEOUT_MS / 1000).toInt()))
    }
