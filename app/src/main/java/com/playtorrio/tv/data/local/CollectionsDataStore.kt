...
import com.playtorrio.tv.domain.model.Collection
import com.playtorrio.tv.domain.model.CollectionCatalogSource
...
import com.playtorrio.tv.domain.model.TraktListSort
import com.playtorrio.tv.domain.model.TraktSortHow
...
class CollectionsDataStore @Inject constructor(
    ...
    fun validateCollectionsJson(json: String): ValidationResult {
        if (json.isBlank()) return ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_empty_input))
        return try {
            val type = object : TypeToken<List<Map<String, Any?>>>() {}.type
            val parsed = gson.fromJson<List<Map<String, Any?>>>(json, type)
                ?: return ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_expected_array))
            if (parsed.isEmpty()) return ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_empty_array))
            ...
            for ((i, item) in parsed.withIndex()) {
                ...
                if (id.isNullOrBlank()) return ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_missing_id, i + 1))
                val title = item["title"] as? String
                    ?: return ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_missing_title, id))
                val folders = item["folders"] as? List<*>
                    ?: return ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_folders_array, title))

                for ((j, f) in folders.withIndex()) {
                    val folder = f as? Map<*, *>
                        ?: return ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_folder_invalid, title, j + 1))
                    val folderId = folder["id"] as? String
                    if (folderId.isNullOrBlank()) return ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_folder_missing_id, title, j + 1))
                    val folderTitle = folder["title"] as? String
                        ?: return ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_folder_missing_title, title, folderId))
                    val sources = (folder["sources"] as? List<*>) ?: (folder["catalogSources"] as? List<*>)
                        ?: return ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_sources_array, title, folderTitle))
                    val shape = folder["tileShape"] as? String
                    if (shape != null && shape !in validShapes) {
                        return ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_invalid_tile_shape, title, folderTitle, shape))
                    }
                    for ((k, s) in sources.withIndex()) {
                        val source = s as? Map<*, *>
                            ?: return ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_source_invalid, title, folderTitle, k + 1))
                        ...
                        if (isAddonSource && (source["addonId"] !is String || source["type"] !is String || source["catalogId"] !is String)) {
                            return ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_source_required_fields, title, folderTitle, k + 1))
                        }
                        if (isTmdbSource && source["tmdbSourceType"] !is String) {
                            return ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_missing_tmdb_type, title, folderTitle, k + 1))
                        }
                        if (isTraktSource && (source["traktListId"] as? Number)?.toLong() == null) {
                            return ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_missing_trakt_list_id, title, folderTitle, k + 1))
                        }
                    }
                    folderCount++
                }
            ...
            ValidationResult(false, appContext.getString(com.playtorrio.tv.R.string.collections_import_error_json_parse, e.message.orEmpty()))
        }
    }
