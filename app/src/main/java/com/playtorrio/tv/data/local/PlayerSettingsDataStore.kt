...
data class PlayerSettings(
    ...
    // PlayTorrio ExoPlayer Performance Mode
    val playtorrioPerformanceModeEnabled: Boolean = DEFAULT_PLAYTORRIO_PERFORMANCE_MODE_ENABLED
) {
    ...
    companion object {
        ...
        const val DEFAULT_PLAYTORRIO_PERFORMANCE_MODE_ENABLED = false
    }
}
 ...
 * routes via [com.playtorrio.tv.core.player.DolbyVisionBaseLayerPolicy].
 *
...
class PlayerSettingsDataStore @Inject constructor(
    ...
    private val playtorrioPerformanceModeEnabledKey = booleanPreferencesKey("playtorrio_performance_mode_enabled")
    ...
    val playerSettings: Flow<PlayerSettings> = profileManager.activeProfileId.flatMapLatest { pid ->
        factory.get(pid, FEATURE).data.onStart { migrateProfile(pid) }
            ...
            PlayerSettings(
                ...
                playtorrioPerformanceModeEnabled = (prefs[playtorrioPerformanceModeEnabledKey] ?: PlayerSettings.DEFAULT_PLAYTORRIO_PERFORMANCE_MODE_ENABLED) &&
    ...
    val playtorrioPerformanceModeEnabled: Flow<Boolean> = profileManager.activeProfileId.flatMapLatest { pid ->
        factory.get(pid, FEATURE).data.map { prefs ->
            (prefs[playtorrioPerformanceModeEnabledKey] ?: PlayerSettings.DEFAULT_PLAYTORRIO_PERFORMANCE_MODE_ENABLED) &&
                    android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O
        }
    }
    ...
    suspend fun setPlayTorrioPerformanceModeEnabled(enabled: Boolean) {
        ...
        store().edit { prefs ->
            prefs[playtorrioPerformanceModeEnabledKey] = actualEnabled
            if (actualEnabled) {
    ...
    private fun isNativeMemoryActive(prefs: androidx.datastore.preferences.core.Preferences): Boolean {
        val isEnabled = prefs[playtorrioPerformanceModeEnabledKey] ?: PlayerSettings.DEFAULT_PLAYTORRIO_PERFORMANCE_MODE_ENABLED
