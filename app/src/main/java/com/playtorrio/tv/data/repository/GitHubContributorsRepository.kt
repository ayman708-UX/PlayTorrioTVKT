...
import com.playtorrio.tv.data.remote.dto.UniqueContributorDto
...
class GitHubContributorsRepository @Inject constructor(
    ...
    suspend fun getContributors(): Result<List<GitHubContributor>> = runCatching {
        if (contributionsBaseUrl.isBlank()) {
            error(appContext.getString(com.playtorrio.tv.R.string.contributors_error_api_not_configured))
        }

        val response = contributionsApi.getUniqueContributions()
        if (!response.isSuccessful) {
            error(appContext.getString(com.playtorrio.tv.R.string.contributors_error_api_http, response.code()))
        }
