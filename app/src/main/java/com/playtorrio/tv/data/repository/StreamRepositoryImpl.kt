...
import com.playtorrio.tv.core.network.safeApiCall
...
class StreamRepositoryImpl @Inject constructor(
    ...
    private fun fetchStreamsFromAllSources(
        ...
        try {
            ...
            coroutineScope {
                    ...
                    launch {
                        try {
                            ...
                            attemptedFailures += StreamAttemptFailure(
                                ...
                                detail = e.message ?: context.getString(com.playtorrio.tv.R.string.stream_error_detail_addon_request_failed)
                            )
                        } finally {
                ...
                launch {
                    try {
                        ...
                        playTorrioHttpScraperManager.scrapeStreamsDynamic(
                            type = type,
                            title = mediaTitle,
                            year = mediaYear,
                            season = pluginSeason,
                            ...
                            if (streamsBatch.isNotEmpty()) {
                                resultChannel.send(
                                    AddonStreams(
                                        addonName = com.playtorrio.tv.core.scraper.PlayTorrioHttpScraperManager.ADDON_NAME,
                                        addonLogo = null,
                        ...
                        if (e is CancellationException) throw e
                        Log.e(TAG, "PlayTorrioHTTP scraping failed: ${e.message}")
                    } finally {
                        if (completedJobs.incrementAndGet() >= totalJobs) {
                            resultChannel.close()
                        }
            ...
            emit(NetworkResult.Error(e.message ?: context.getString(com.playtorrio.tv.R.string.stream_error_fetch_failed)))
        }
    }
     ...
     */
    private fun buildDescription(result: com.playtorrio.tv.domain.model.LocalScraperResult): String? {
    ...
    private fun buildMissingStreamFailure(addon: Addon): StreamAttemptFailure {
        return StreamAttemptFailure(
            ...
            detail = context.getString(com.playtorrio.tv.R.string.stream_error_detail_no_streams_for_id)
        )
    }
    ...
    private fun buildAddonFailure(addon: Addon, error: NetworkResult.Error): StreamAttemptFailure {
        ...
        val normalizedReason = when {
            error.message.contains("Unable to resolve host", ignoreCase = true) ->
                context.getString(com.playtorrio.tv.R.string.stream_error_detail_addon_unreachable)
            error.message.contains("Failed to connect", ignoreCase = true) ->
                context.getString(com.playtorrio.tv.R.string.stream_error_detail_addon_connection_failed)
            error.message.contains("timeout", ignoreCase = true) ->
                context.getString(com.playtorrio.tv.R.string.stream_error_detail_addon_timeout)
            error.message.contains("CLEARTEXT communication", ignoreCase = true) ->
                context.getString(com.playtorrio.tv.R.string.stream_error_detail_addon_cleartext_blocked)
            error.message.isBlank() ->
                context.getString(com.playtorrio.tv.R.string.stream_error_detail_addon_request_failed)
