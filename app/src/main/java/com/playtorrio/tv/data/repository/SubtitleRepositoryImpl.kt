...
import com.playtorrio.tv.data.local.AddonPreferences
...
class SubtitleRepositoryImpl @Inject constructor(
    ...
    private val playTorrioSubManager: com.playtorrio.tv.core.subtitles.PlayTorrioSubManager
) : SubtitleRepository {
    ...
    override suspend fun getSubtitles(
        ...
        val result = supervisorScope {
            ...
            // 2. Built-in PlayTorrioSub manager
            tasks.add(
                async {
                    ...
                    val subtitles = try {
                        ...
                        playTorrioSubManager.fetchSubtitles(
                            type = requestType,
                        ...
                        Log.e(TAG, "PlayTorrioSub fetch failed: ${e.message}")
                        emptyList()
                    }
                    onProgress?.invoke(completedCount.incrementAndGet(), total, com.playtorrio.tv.core.subtitles.PlayTorrioSubManager.ADDON_NAME)
                    Log.d(
                        TAG,
                        "PlayTorrioSub fetch done count=${subtitles.size} in ${System.currentTimeMillis() - subStartMs}ms"
                    )
                    subtitles
                }
            )
    ...
    private fun supportsType(addon: Addon, resource: com.playtorrio.tv.domain.model.AddonResource, type: String, id: String): Boolean {
