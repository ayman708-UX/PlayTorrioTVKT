...
import com.playtorrio.tv.core.tracking.mergeTrackingMembershipWithTabs
        ...
        runCatching {
            ...
            _uiState.update { state ->
                state.copy(
                    showPosterListPicker = true,
                    posterListPickerPending = false,
                    posterListPickerError = error.message ?: appContext.getString(com.playtorrio.tv.R.string.home_poster_lists_error_load_failed)
                )
            }
        }
    }
    ...
    viewModelScope.launch {
        ...
        runCatching {
            ...
            _uiState.update { state ->
                state.copy(
                    posterListPickerPending = false,
                    posterListPickerError = error.message ?: appContext.getString(com.playtorrio.tv.R.string.home_poster_lists_error_update_failed)
                )
            }
        }
    }
            ...
            _uiState.update {
                it.copy(
                    ...
                    posterListPickerError = error.message
                        ?: appContext.getString(com.playtorrio.tv.R.string.home_poster_lists_error_update_failed)
                )
            }
        }
    }
...
private suspend fun HomeViewModel.fetchSeriesEpisodes(item: MetaPreview): List<com.playtorrio.tv.domain.model.Video> {
    val type = if (item.apiType.equals("tv", ignoreCase = true)) "series" else item.apiType
    var episodes: List<com.playtorrio.tv.domain.model.Video> = emptyList()
    metaRepository.getMetaFromPrimaryAddon(type, item.id)
        .collect { networkResult ->
            if (networkResult is com.playtorrio.tv.core.network.NetworkResult.Success) {
