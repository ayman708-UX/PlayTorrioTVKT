...
private fun AudioTracksContent(
    ...
    if (tracks.isEmpty()) {
        Text(
            ...
            modifier = Modifier.padding(top = PlayTorrioTheme.spacing.sm, bottom = PlayTorrioTheme.spacing.md)
        )
        return
    }
...
private fun AudioTrackCard(
    ...
    Card(
        ...
        colors = CardDefaults.colors(
            containerColor = if (isSelected) PlayTorrioTheme.colors.Secondary else Color.Transparent,
            focusedContainerColor = if (isSelected) PlayTorrioTheme.colors.Secondary else Color.Transparent
        ),
        ...
        val primaryTextColor = if (isSelected) PlayTorrioTheme.colors.OnSecondary else Color.White
        val secondaryTextColor = if (isSelected) {
            PlayTorrioTheme.colors.OnSecondary.copy(alpha = 0.82f)
        } else {
...
private fun AudioControlsContent(
    ...
    Column(
        modifier = Modifier
            .fillMaxWidth()
            .padding(top = PlayTorrioTheme.spacing.xs, bottom = PlayTorrioTheme.spacing.sm),
        ...
        Column(
            ...
            Card(
                ...
                colors = CardDefaults.colors(
                    containerColor = if (persistAmplification) PlayTorrioTheme.colors.Secondary else Color.Transparent,
                    focusedContainerColor = if (persistAmplification) PlayTorrioTheme.colors.Secondary else Color.Transparent
                ),
                shape = CardDefaults.shape(RoundedCornerShape(PlayTorrioTheme.radii.md)),
                border = CardDefaults.border(
                    border = Border(
                ...
                Text(
                    ...
                    style = MaterialTheme.typography.bodySmall,
                    color = if (persistAmplification) PlayTorrioTheme.colors.OnSecondary else Color.White,
...
private fun StepCard(
    ...
    Card(
        ...
        border = CardDefaults.border(
            border = Border(
                border = BorderStroke(PlayTorrioTheme.spacing.xxs, if (enabled) Color.White.copy(alpha = 0.18f) else Color.Transparent),
                shape = RoundedCornerShape(PlayTorrioTheme.radii.md)
            ),
            focusedBorder = Border(
                border = PlayTorrioTheme.focusRing.border(
