...
import com.playtorrio.tv.core.player.SubtitleCharsetDetector
import com.playtorrio.tv.core.player.dvmkv.MatroskaExtractor
import com.playtorrio.tv.core.player.dvmkv.MatroskaExtractor.DolbyVisionSampleTransformer
...
internal class PlayTorrioAssMatroskaExtractor(
    ...
    private fun onEbmlStart(contentPosition: Long, contentSize: Long) {
        if (assHandler.renderType != AssRenderType.CUES) {
            ...
            if (currentExtractor !is PlayTorrioAssSubtitleExtractorOutput) {
                extractorOutput.set(
                    this,
                    PlayTorrioAssSubtitleExtractorOutput(currentExtractor, assHandler, this)
                )
            }
        }
...
@OptIn(UnstableApi::class)
private class PlayTorrioAssSubtitleExtractorOutput(
    private val delegate: ExtractorOutput,
    private val assHandler: AssHandler,
    private val extractor: PlayTorrioAssMatroskaExtractor
) : ExtractorOutput by delegate {
    override fun track(id: Int, type: Int): TrackOutput {
        return if (type == C.TRACK_TYPE_TEXT) {
            PlayTorrioAssTrackOutput(delegate.track(id, type), assHandler, extractor)
        } else {
...
@OptIn(UnstableApi::class)
private class PlayTorrioAssTrackOutput(
