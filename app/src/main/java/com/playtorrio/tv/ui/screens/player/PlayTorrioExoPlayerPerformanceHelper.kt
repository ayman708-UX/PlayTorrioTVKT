...
/**
 * Centralizes all PlayTorrio ExoPlayer performance enhancements behind a single toggle.
 *
...
object PlayTorrioExoPlayerPerformanceHelper {

    /** Whether PlayTorrio performance enhancements are active. Set from [PlayerSettingsDataStore]. */
    @Volatile
    ...
    var sharedConnectionPool: okhttp3.ConnectionPool = okhttp3.ConnectionPool(
        DEFAULT_PLAYTORRIO_CONNECTION_POOL_SIZE,
        3,
    ...
    // ─── Constants ────────────────────────────────────────────────────────────
    const val DEFAULT_PLAYTORRIO_ALLOCATOR_SEGMENT_SIZE = 64 * 1024        // 64 KB
    const val DEFAULT_PLAYTORRIO_TARGET_BUFFER_BYTES = 250 * 1024 * 1024    // 250 MB
    const val DEFAULT_PLAYTORRIO_MIN_BUFFER_MS = 40_000
    const val DEFAULT_PLAYTORRIO_MAX_BUFFER_MS = 120_000
    const val DEFAULT_PLAYTORRIO_BACK_BUFFER_MS = 1_500
    const val DEFAULT_PLAYTORRIO_INITIAL_BITRATE_ESTIMATE = 50_000_000L     // 50 Mbps
    const val DEFAULT_PLAYTORRIO_CONNECTION_POOL_SIZE = 8

    // ─── Customization Variables ──────────────────────────────────────────────
    @Volatile
    var minBufferMs: Int = DEFAULT_PLAYTORRIO_MIN_BUFFER_MS

    @Volatile
    var maxBufferMs: Int = DEFAULT_PLAYTORRIO_MAX_BUFFER_MS
    ...
    @Volatile
    var backBufferMs: Int = DEFAULT_PLAYTORRIO_BACK_BUFFER_MS

    @Volatile
    var targetBufferSizeMb: Int = 250

    @Volatile
    var connectionPoolSize: Int = DEFAULT_PLAYTORRIO_CONNECTION_POOL_SIZE
    ...
    fun updateSettings(settings: PlayerSettings, context: Context) {
        ...
        minBufferMs = if (customBuffers) bufferSettings.minBufferMs else DEFAULT_PLAYTORRIO_MIN_BUFFER_MS
        maxBufferMs = if (customBuffers) bufferSettings.maxBufferMs else DEFAULT_PLAYTORRIO_MAX_BUFFER_MS
        ...
        backBufferMs = if (customBuffers) bufferSettings.backBufferDurationMs else DEFAULT_PLAYTORRIO_BACK_BUFFER_MS
    ...
    fun buildLoadControl(context: Context? = null): DefaultLoadControl {
        return if (enabled) {
            ...
            DefaultLoadControl.Builder()
                .setAllocator(DefaultAllocator(true, DEFAULT_PLAYTORRIO_ALLOCATOR_SEGMENT_SIZE, 64, enabled))
    ...
    fun buildBandwidthMeter(context: Context): DefaultBandwidthMeter {
        return if (enabled) {
            DefaultBandwidthMeter.Builder(context)
                .setInitialBitrateEstimate(DEFAULT_PLAYTORRIO_INITIAL_BITRATE_ESTIMATE)
                .build()
        } else {
    ...
    /**
     * Applies [PlayTorrioEngineConfig] based on the toggle state.
