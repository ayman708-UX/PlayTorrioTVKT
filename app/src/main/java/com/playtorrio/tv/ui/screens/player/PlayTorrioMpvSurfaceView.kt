...
class PlayTorrioMpvSurfaceView @JvmOverloads constructor(
    context: Context,
    ...
    fun setMediaUsingLoadfile(url: String, headers: Map<String, String>) {
        ...
        val requestKey = buildMediaRequestKey(url = url, headers = resolvedHeaders)
        applyHeaders(resolvedHeaders)
        pendingInitialMediaUrl = null
        pendingInitialStartOption = null
        if (holder.surface?.isValid == true) {
            ensureSurfaceAttachedIfAlreadyAvailable()
            ...
            when (type) {
                "audio" -> {
                    audioTracks += MpvTrack(
                        id = id,
                        type = type,
                        name = title ?: language ?: context.getString(com.playtorrio.tv.R.string.player_track_audio_fallback, id),
                        language = language,
                        codec = codec,
                ...
                "sub" -> {
                    subtitleTracks += MpvTrack(
                        id = id,
                        type = type,
                        name = title ?: language ?: context.getString(com.playtorrio.tv.R.string.player_track_subtitle_fallback, id),
                        language = language,
                        codec = codec,
                        channelCount = null,
                        isSelected = selected,
    ...
    private fun applyHeaders(headers: Map<String, String>) {
        ...
        headers.entries.firstOrNull { it.key.equals("Referer", ignoreCase = true) }?.value?.let {
            mpv.setPropertyString("referrer", it)
        }
        headers.entries.firstOrNull { it.key.equals("User-Agent", ignoreCase = true) }?.value?.let {
            mpv.setPropertyString("user-agent", it)
        }
    }
    ...
    companion object {
        private const val TAG = "PlayTorrioMpvSurfaceView"
