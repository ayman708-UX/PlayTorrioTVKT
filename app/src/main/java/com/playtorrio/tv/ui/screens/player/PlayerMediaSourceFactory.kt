...
import com.playtorrio.tv.PlayTorrioApplication
...
internal class PlayerMediaSourceFactory(private val context: Context) {
    ...
    var playtorrioPerformanceModeEnabled: Boolean = PlayerSettings.DEFAULT_PLAYTORRIO_PERFORMANCE_MODE_ENABLED
    ...
    private val playbackHttpClient by lazy {
        PlayerPlaybackNetworking.playbackHttpClient.newBuilder()
            .cookieJar(PlayTorrioApplication.extensionCookieJar)
            .let { PlayTorrioExoPlayerPerformanceHelper.applyNetworkOptimizations(it) }
            .build()
    }
        ...
        val progressiveUpstreamFactory: DataSource.Factory = if (useChunkSessionSource) {
            ...
            ParallelRangeDataSource.Factory(
                ...
                if (mp4SessionMode) {
                    ...
                    // must not bypass it.
                    parallelChunkSizeKb
                        .coerceAtMost(com.playtorrio.tv.ui.screens.settings.MemoryBudget.tierMaxChunkMb * 1024)
                        .toLong() * 1024L
                },
                useNativeMemory = playtorrioPerformanceModeEnabled,
