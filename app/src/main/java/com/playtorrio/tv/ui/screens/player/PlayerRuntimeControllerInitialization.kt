...
import com.playtorrio.tv.core.player.DolbyVisionCodecFallback
import com.playtorrio.tv.core.player.DolbyVisionBaseLayerPolicy
import com.playtorrio.tv.core.player.BitrateAwareLoadControl
import com.playtorrio.tv.core.player.DolbyVisionConversionConfig
import com.playtorrio.tv.core.player.DolbyVisionConversionStats
import com.playtorrio.tv.core.player.DolbyVisionExtractorsFactory
import com.playtorrio.tv.core.player.DoviBridge
import com.playtorrio.tv.core.player.LastPlaybackDiagnostics
import com.playtorrio.tv.core.tracking.TrackingScrobbleAction
import com.playtorrio.tv.ui.screens.settings.MemoryBudget
import com.playtorrio.tv.data.local.AudioLanguageOption
import com.playtorrio.tv.data.local.Dv7HandlingMode
import com.playtorrio.tv.data.local.FrameRateMatchingMode
import com.playtorrio.tv.data.local.SUBTITLE_LANGUAGE_FORCED
import com.playtorrio.tv.data.local.InternalPlayerEngine
import com.playtorrio.tv.data.local.PlayerSettings
import com.playtorrio.tv.data.repository.PlaybackIssueErrorInput
...
internal fun PlayerRuntimeController.initializePlayer(
    ...
    scope.launch {
        try {
            ...
            PlayTorrioExoPlayerPerformanceHelper.updateSettings(playerSettings, context)
            PlayTorrioExoPlayerPerformanceHelper.enabled = playerSettings.playtorrioPerformanceModeEnabled
            ...
            val rawBandwidthMeter = if (PlayTorrioExoPlayerPerformanceHelper.enabled) {
                PlayTorrioExoPlayerPerformanceHelper.buildBandwidthMeter(context)
            } else {
            ...
            val loadControl = if (playerSettings.playtorrioPerformanceModeEnabled) {
                ...
                Log.i(
                    PlayerRuntimeController.TAG,
                    "BUFFER_GATE: engine=exo-native-perf master=on; PlayTorrioExoPlayerPerformanceHelper.buildLoadControl host=${url.safeHost()}"
                )
                PlayTorrioExoPlayerPerformanceHelper.buildLoadControl(context)
            ...
            if (playerSettings.parallelNetworkEnabled) {
                ...
                mediaSourceFactory.playtorrioPerformanceModeEnabled = playerSettings.playtorrioPerformanceModeEnabled
            } else {
                // Reset each playback so the factory doesn't keep last stream's state.
                mediaSourceFactory.useParallelConnections = false
                mediaSourceFactory.playtorrioPerformanceModeEnabled = false
            }
            ...
            trackSelector = object : DefaultTrackSelector(context, adaptiveTrackSelectionFactory) {
                override fun selectAllTracks(
                    ...
                    Log.d("PlayTorrioTrackSelector", "selectAllTracks run: streamMime=$streamMime, isHls=$isHls")
                    if (isHls) {
                            ...
                            if (mappedTrackInfo.getRendererType(rendererIndex) == C.TRACK_TYPE_VIDEO) {
                                ...
                                for (groupIndex in 0 until trackGroups.length) {
                                    ...
                                    for (trackIndex in 0 until group.length) {
                                        ...
                                        Log.d("PlayTorrioTrackSelector", "Evaluating track: id=${format.id}, res=${format.width}x${format.height}, mime=${format.sampleMimeType}, codecs=${format.codecs}, support=${formatSupport}")
                                        if (formatSupport == C.FORMAT_EXCEEDS_CAPABILITIES) {
                                            ...
                                            Log.d("PlayTorrioTrackSelector", "Conditions for id=${format.id}: isAvcOrHevc=$isAvcOrHevc, isAtMost1080p=$isAtMost1080p, isStandard8Bit=$isStandard8Bit")
                                            if (isAvcOrHevc && isAtMost1080p && isStandard8Bit) {
                                                Log.i("PlayTorrioTrackSelector", "Upgraded track support to FORMAT_HANDLED for id=${format.id}")
            ...
            com.playtorrio.tv.core.player.dvmkv.DolbyVisionCompatibility.setHdr10BaseLayerModeActive(isHdr10BaseLayerModeActive)
            ...
            val effectiveAudioOutputChannels = if (bluetoothStereoDownmix) {
                com.playtorrio.tv.data.local.AudioOutputChannels.CHANNELS_2_0
            } else {
            ...
            _exoPlayer?.apply {
                ...
                addListener(object : Player.Listener {
                    override fun onPlaybackStateChanged(playbackState: Int) {
                        ...
                        _uiState.update {
                            it.copy(
                                isBuffering = if (PlayTorrioExoPlayerPerformanceHelper.shouldSuppressBufferingUi(
                        ...
                        if (playbackState == Player.STATE_READY) {
                            ...
                            _uiState.value.pendingSeekPosition?.let { position ->
                                seekTo(position)
                                if (PlayTorrioExoPlayerPerformanceHelper.enabled) {
                            ...
                            if (!PlayTorrioExoPlayerPerformanceHelper.shouldGuardTrackRebuild() || !hasRenderedFirstFrame) {
            ...
            if (
                maybeAutoSwitchInternalPlayerOnStartupError(
                    detailedError = e.message ?: context.getString(com.playtorrio.tv.R.string.player_error_initialize_failed),
                    allowEngineFailover = allowEngineFailover
                )
            ) {
                return@launch
            }
            val displayError = e.toDisplayMessage(context, context.getString(com.playtorrio.tv.R.string.player_error_initialize_failed))
...
private class SubtitleOffsetRenderersFactory(
    ...
    private val audioOutputChannels: com.playtorrio.tv.data.local.AudioOutputChannels,
...
private fun FfmpegAudioRenderer.applyDownmixSettings(
    ...
    audioOutputChannels: com.playtorrio.tv.data.local.AudioOutputChannels,
...
private fun PlayerRuntimeController.recordFirstFrameDiagnostics(
    ...
    playerSettings: com.playtorrio.tv.data.local.PlayerSettings
