...
internal fun PlayerRuntimeController.attachMpvView(view: PlayTorrioMpvSurfaceView?) {
    ...
    runCatching {
        ...
        val detailedError = it.message ?: context.getString(com.playtorrio.tv.R.string.player_error_mpv_surface_failed)
        if (
...
internal fun PlayerRuntimeController.initializeMpvPlayer(
    ...
    if (view == null) {
        setLoadingStatus(
            ...
            message = context.getString(com.playtorrio.tv.R.string.player_loading_building),
            showOverlay = true
        )
        _uiState.update {
            it.copy(
    ...
    runCatching {
        setLoadingStatus(
            phase = "mpv_starting",
            message = context.getString(com.playtorrio.tv.R.string.player_loading_starting),
            showOverlay = true
        )
        performPendingMpvHardRestartIfNeeded(view)
        view.applyHardwareDecodeMode(mpvHardwareDecodeModeSetting)
        ...
        val detailedError = error.message ?: context.getString(com.playtorrio.tv.R.string.player_error_mpv_playback_failed)
        if (
            maybeAutoSwitchInternalPlayerOnStartupError(
                detailedError = detailedError,
...
private fun PlayerRuntimeController.performPendingMpvHardRestartIfNeeded(view: PlayTorrioMpvSurfaceView): Boolean {
    ...
    if (isUsingMpvEngine()) {
        ...
        _exoPlayer?.let { player ->
            if (PlayTorrioExoPlayerPerformanceHelper.enabled) {
                ...
                val inBuffer = isForwardSeek && PlayTorrioExoPlayerPerformanceHelper.isSeekInBuffer(player, positionMs)
                if (inBuffer) {
                ...
                PlayTorrioExoPlayerPerformanceHelper.buildScrubbingParams()?.let { params ->
...
internal fun PlayerRuntimeController.scheduleSeekSuppressTimeout() {
    scope.launch {
        delay(PlayTorrioExoPlayerPerformanceHelper.SEEK_SUPPRESS_TIMEOUT_MS)
        suppressBufferingUiForSeek = false
    }
}
