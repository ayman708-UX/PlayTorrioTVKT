...
import com.playtorrio.tv.core.debrid.DirectDebridPlayableResult
import com.playtorrio.tv.core.network.NetworkResult
import com.playtorrio.tv.core.player.StreamAutoPlaySelector
import com.playtorrio.tv.data.local.PlayerSettings
import com.playtorrio.tv.data.local.StreamAutoPlayMode
import com.playtorrio.tv.data.local.StreamAutoPlaySource
import com.playtorrio.tv.data.local.toTrackPreference
...
internal fun PlayerRuntimeController.scheduleSourceBadgeApplication() {
    ...
    sourceBadgeJob = scope.launch(kotlinx.coroutines.Dispatchers.Default) {
        ...
        for (chunk in chunks) {
            val chunkGroup = com.playtorrio.tv.domain.model.AddonStreams(addonName = "", addonLogo = null, streams = chunk)
...
internal fun PlayerRuntimeController.scheduleEpisodeBadgeApplication() {
    ...
    episodeBadgeJob = scope.launch(kotlinx.coroutines.Dispatchers.Default) {
        ...
        val group = com.playtorrio.tv.domain.model.AddonStreams(addonName = "", addonLogo = null, streams = streams)
...
private suspend fun PlayerRuntimeController.updateSourceChipsForFetchStart(
    type: String,
    videoId: String,
    installedAddons: List<com.playtorrio.tv.domain.model.Addon>
) {
...
private fun com.playtorrio.tv.domain.model.Addon.supportsStreamResourceForChip(type: String, videoId: String): Boolean {
...
private fun PlayerRuntimeController.openExternalStreamInBrowser(
    ...
    if (externalUrl.isNullOrBlank()) {
        _uiState.update {
            if (fromEpisodePanel) {
                it.copy(episodeStreamsError = context.getString(com.playtorrio.tv.R.string.player_stream_error_invalid_external_url))
            } else {
                it.copy(sourceStreamsError = context.getString(com.playtorrio.tv.R.string.player_stream_error_invalid_external_url))
            }
        }
        return true
    }
    ...
    runCatching {
        ...
        _uiState.update {
            if (fromEpisodePanel) {
                it.copy(episodeStreamsError = error.message ?: context.getString(com.playtorrio.tv.R.string.player_stream_error_open_external_link_failed))
            } else {
                it.copy(sourceStreamsError = error.message ?: context.getString(com.playtorrio.tv.R.string.player_stream_error_open_external_link_failed))
            }
        }
    }
...
internal fun PlayerRuntimeController.switchToSourceStream(
        ...
        debridResolveJob = scope.launch {
            ...
            if (resolved != null && !resolved.getStreamUrl().isNullOrBlank()) {
                ...
                _uiState.update {
                    it.copy(
                        isLoadingSourceStreams = false,
                        sourceStreamsError = context.getString(com.playtorrio.tv.R.string.player_stream_error_invalid_url)
                    )
                }
            }
        }
    ...
    if (url.isNullOrBlank()) {
        ...
        _uiState.update { it.copy(sourceStreamsError = context.getString(com.playtorrio.tv.R.string.player_stream_error_invalid_url)) }
        return
    }
    ...
    _exoPlayer?.let { player ->
        scope.launch {
            try {
                ...
                _uiState.update { it.copy(error = e.message ?: context.getString(com.playtorrio.tv.R.string.player_error_play_stream_failed)) }
            }
        }
    } ?: run {
...
internal fun PlayerRuntimeController.loadStreamsForEpisode(video: Video, forceRefresh: Boolean) {
    ...
    if (type.isNullOrBlank()) {
        _uiState.update { it.copy(episodeStreamsError = context.getString(com.playtorrio.tv.R.string.player_stream_error_missing_content_type)) }
        return
    }
...
internal fun PlayerRuntimeController.switchToEpisodeStream(
        ...
        debridResolveJob = scope.launch {
            ...
            if (resolved != null && !resolved.getStreamUrl().isNullOrBlank()) {
                ...
                _uiState.update {
                    it.copy(
                        isLoadingEpisodeStreams = false,
                        episodeStreamsError = context.getString(com.playtorrio.tv.R.string.player_stream_error_invalid_url)
                    )
                }
            }
        }
    ...
    if (url.isNullOrBlank()) {
        ...
        _uiState.update { it.copy(episodeStreamsError = context.getString(com.playtorrio.tv.R.string.player_stream_error_invalid_url)) }
        return
    }
...
internal suspend fun PlayerRuntimeController.resolveDirectDebridStreamIfNeeded(
    ...
    recordLoadingDiagnosticEvent(
        ...
        message = context.getString(com.playtorrio.tv.R.string.player_loading_preparing),
    ...
    nextEpisodeAutoPlayJob = scope.launch {
        try {
            when (val result = withTimeoutOrNull(CLOUD_LIBRARY_AUTO_NEXT_TIMEOUT_MS) {
                ...
                is com.playtorrio.tv.core.cloud.CloudLibraryPlaybackResult.Success -> {
                    ...
                    val stream = Stream(
                        ...
                        fileIdx = null,
                        externalUrl = null,
                        behaviorHints = com.playtorrio.tv.domain.model.StreamBehaviorHints(
                            notWebReady = null,
                            bingeGroup = null,
                    ...
                    _uiState.update {
                        it.copy(
                            postPlayMode = null,
                            postPlayDismissedForCurrentEpisode = true,
                            error = context.getString(com.playtorrio.tv.R.string.cloud_library_play_failed)
                        )
                    }
                }
            }
