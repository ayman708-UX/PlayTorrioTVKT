...
internal suspend fun PlayerRuntimeController.startTorrentStream(
    ...
    setLoadingStatus(
        ...
        message = context.getString(com.playtorrio.tv.R.string.player_torrent_starting_engine),
        showOverlay = true
    )
    _uiState.update {
        it.copy(
            showLoadingOverlay = true,
            loadingMessage = context.getString(com.playtorrio.tv.R.string.player_torrent_starting_engine),
        ...
        torrentService.state.collectLatest { torrentState ->
            when (torrentState) {
                ...
                is TorrentState.Connecting -> {
                    if (!hasRenderedFirstFrame) {
                        recordLoadingDiagnosticEvent(
                            ...
                            message = context.getString(com.playtorrio.tv.R.string.player_torrent_connecting_peers)
                        )
                        _uiState.update {
                            it.copy(
                                showLoadingOverlay = true,
                                loadingMessage = context.getString(com.playtorrio.tv.R.string.player_torrent_connecting_peers),
                                loadingProgress = null,
                ...
                is TorrentState.Streaming -> {
                    ...
                    val peerInfo = context.getString(com.playtorrio.tv.R.string.player_torrent_peer_info, torrentState.seeds, torrentState.peers)
                    ...
                    if (!hasRenderedFirstFrame) {
                        ...
                        val message = if (statsHidden) null else context.getString(com.playtorrio.tv.R.string.player_torrent_buffered_status, mbLoaded, peerInfo, speed)
                        recordLoadingDiagnosticEvent(
                        ...
                        val message = if (statsHidden) null else context.getString(com.playtorrio.tv.R.string.player_torrent_status, peerInfo, speed)
                        _uiState.update {
                            it.copy(
                                loadingProgress = null,
                                torrentDownloadSpeed = torrentState.downloadSpeed,
                ...
                is TorrentState.Error -> {
                    ...
                    _uiState.update {
                        it.copy(
                            error = context.getString(com.playtorrio.tv.R.string.player_error_torrent, torrentState.message),
...
private fun formatSpeed(context: android.content.Context, bytesPerSec: Long): String {
    return when {
        bytesPerSec >= 1_048_576 -> context.getString(com.playtorrio.tv.R.string.unit_speed_mb_s, String.format("%.1f", bytesPerSec / 1_048_576.0))
        bytesPerSec >= 1_024 -> context.getString(com.playtorrio.tv.R.string.unit_speed_kb_s, String.format("%.0f", bytesPerSec / 1_024.0))
        else -> context.getString(com.playtorrio.tv.R.string.unit_speed_b_s, bytesPerSec)
    }
}

private fun formatMB(context: android.content.Context, bytes: Long): String =
    context.getString(com.playtorrio.tv.R.string.unit_size_mb, String.format("%.1f", bytes / 1_048_576.0))
