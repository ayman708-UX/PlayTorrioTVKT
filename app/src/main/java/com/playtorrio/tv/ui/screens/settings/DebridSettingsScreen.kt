...
import com.playtorrio.tv.core.debrid.DebridDeviceAuthorization
import com.playtorrio.tv.core.debrid.DebridDeviceAuthorizationTokenResult
import com.playtorrio.tv.core.debrid.DebridProvider
import com.playtorrio.tv.core.debrid.DebridProviderAuthMethod
import com.playtorrio.tv.core.debrid.DebridProviders
import com.playtorrio.tv.core.qr.QrCodeGenerator
import com.playtorrio.tv.ui.components.LoadingIndicator
import com.playtorrio.tv.domain.model.DebridStreamAudioChannel
import com.playtorrio.tv.domain.model.DebridStreamAudioTag
import com.playtorrio.tv.domain.model.DebridStreamEncode
import com.playtorrio.tv.domain.model.DebridStreamLanguage
import com.playtorrio.tv.domain.model.DebridStreamPreferences
import com.playtorrio.tv.domain.model.DebridStreamQuality
...
import com.playtorrio.tv.ui.screens.addon.QrCodeOverlay
        ...
        SettingsGroupCard(
            ...
            Box(modifier = Modifier.fillMaxSize()) {
                LazyColumn(
                    ...
                    contentPadding = PaddingValues(bottom = PlayTorrioTheme.spacing.sm),
                    ...
                    item(key = "debrid_phone_qr_config") {
                        SettingsActionRow(
                            title = "Configure on Phone (QR Code)",
                            subtitle = "Scan QR code to enter API keys and manage Debrid from your phone",
                            value = "Open",
                    ...
                    DebridProviders.visible().forEach { provider ->
                        item(key = "debrid_${provider.id}_api_key") {
                            SettingsActionRow(
                                ...
                                subtitle = providerHelpUrl(provider.id),
                    ...
                    if (uiState.hasCloudLibraryProvider) {
                        item(key = "debrid_cloud_library") {
                            SettingsToggleRow(
                                ...
                                modifier = Modifier.padding(top = PlayTorrioTheme.spacing.xxs),
                                enabled = true
                            )
                        }
                    }
...
private fun DebridInfoText(text: String) {
    Text(
        ...
        modifier = Modifier
            .fillMaxWidth()
            .padding(horizontal = 18.dp, vertical = PlayTorrioTheme.spacing.sm)
    )
}
...
private fun DebridSectionLabel(text: String) {
    Text(
        ...
        modifier = Modifier.padding(start = PlayTorrioTheme.spacing.sm, top = PlayTorrioTheme.spacing.sm)
    )
}
        ...
        Card(
            ...
            Box(modifier = Modifier.padding(14.dp)) {
                BasicTextField(
                    ...
                    textStyle = MaterialTheme.typography.bodyMedium.copy(color = PlayTorrioTheme.colors.TextPrimary),
                    cursorBrush = SolidColor(PlayTorrioTheme.colors.Primary)
                )
            }
        }
...
private fun DebridDeviceAuthDialog(
    ...
    PlayTorrioDialog(
        ...
        if (isConnected) {
            ...
            Row(
                modifier = Modifier.fillMaxWidth(),
                horizontalArrangement = Arrangement.spacedBy(PlayTorrioTheme.spacing.md, Alignment.CenterHorizontally),
            ...
            statusMessage?.let { message ->
                Row(
                    modifier = Modifier.fillMaxWidth(),
                    horizontalArrangement = Arrangement.spacedBy(PlayTorrioTheme.spacing.sm, Alignment.CenterHorizontally),
                    verticalAlignment = Alignment.CenterVertically
                ) {
                    if (isPolling) {
...
private fun DebridApiKeyDialog(
    ...
    PlayTorrioDialog(
        ...
        Card(
            ...
            Box(modifier = Modifier.padding(horizontal = 14.dp, vertical = PlayTorrioTheme.spacing.md)) {
                BasicTextField(
                    value = value,
                    onValueChange = { value = it },
                    modifier = Modifier
                    ...
                    cursorBrush = SolidColor(
                        if (isInputFocused) PlayTorrioTheme.colors.Primary else Color.Transparent
                    ),
...
): String = maskDebridApiKey(credential, notSetLabel)

private fun providerHelpUrl(providerId: String): String {
    return when (providerId.lowercase()) {
        DebridProviders.TORBOX_ID -> "Get key from torbox.app/settings"
        DebridProviders.PREMIUMIZE_ID -> "Get key from premiumize.me/account"
        DebridProviders.REAL_DEBRID_ID -> "Get token from real-debrid.com/apitoken"
        DebridProviders.ALL_DEBRID_ID -> "Get key from alldebrid.com/apikeys"
        DebridProviders.DEBRID_LINK_ID -> "Get key from debrid-link.com/webapp/register"
        else -> "Enter API key"
    }
}
