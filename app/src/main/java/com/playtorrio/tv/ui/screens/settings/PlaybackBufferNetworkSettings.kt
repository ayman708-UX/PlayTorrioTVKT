...
import com.playtorrio.tv.data.local.VodCacheSizeMode
...
internal fun LazyListScope.bufferAndNetworkSettingsItems(
    ...
    onSetPlayTorrioPerformanceModeEnabled: (Boolean) -> Unit,
    ...
    item(key = "buffer_net_playtorrio_performance_mode") {
        var showWarning by remember { mutableStateOf(false) }

        Column {
            ToggleSettingsItem(
                icon = Icons.Default.Speed,
                title = stringResource(R.string.playback_net_playtorrio_performance_mode),
                subtitle = stringResource(R.string.playback_net_playtorrio_performance_mode_sub),
                isChecked = isSupported && playerSettings.playtorrioPerformanceModeEnabled,
                enabled = true,
            ...
            if (!isSupported && showWarning) {
                Text(
                    text = stringResource(R.string.playback_net_playtorrio_performance_mode_not_supported),
                    style = MaterialTheme.typography.bodySmall,
                    color = Color(0xFFF44336), // Red color
                    modifier = Modifier.padding(start = 52.dp, end = PlayTorrioTheme.spacing.lg, top = PlayTorrioTheme.spacing.xs, bottom = PlayTorrioTheme.spacing.sm)
                )
            } else if (isSupported && playerSettings.playtorrioPerformanceModeEnabled) {
                val context = LocalContext.current
                val ramLabel = PlayTorrioExoPlayerPerformanceHelper.getFriendlyRamLabel(context)
                val safeLimitMb = PlayTorrioExoPlayerPerformanceHelper.getSafeNativeMemoryLimitMb(context)
    ...
    if (playerSettings.bufferEngineEnabled) {
        val isNativeMemory = playerSettings.playtorrioPerformanceModeEnabled
        ...
        item(key = "buffer_net_custom_buffers_header") {
            Text(
                ...
                color = PlayTorrioTheme.colors.TextSecondary,
                modifier = Modifier.padding(vertical = PlayTorrioTheme.spacing.sm)
            )
        }
        ...
        item(key = "buffer_net_target_size") {
            ...
            val context = LocalContext.current
            val safeMaxMb = if (playerSettings.playtorrioPerformanceModeEnabled) {
                PlayTorrioExoPlayerPerformanceHelper.getSafeNativeMemoryLimitMb(context)
            } else {
                MemoryBudget.maxBufferMb(parallelOverheadMb)
            }
            val warningMaxMb = if (playerSettings.playtorrioPerformanceModeEnabled) {
            ...
            val bufferSizeMb = if (playerSettings.playtorrioPerformanceModeEnabled && budgetManaged) {
                safeMaxMb
            } else {
                MemoryBudget
    ...
    if (playerSettings.bufferEngineEnabled || playerSettings.playtorrioPerformanceModeEnabled) {
        ...
        if (playerSettings.vodCacheEnabled) {
            ...
            item(key = "buffer_net_auto_cache_size") {
                ...
                Box(modifier = Modifier.padding(start = PlayTorrioTheme.spacing.xxl)) {
                    ToggleSettingsItem(
                        icon = Icons.Default.Tune,
        ...
        if (playerSettings.useParallelConnections) {
            item(key = "buffer_net_parallel_connection_count") {
                SliderSettingsItem(
                    ...
                    maxValue = if (playerSettings.playtorrioPerformanceModeEnabled) 16 else MemoryBudget.MAX_CONNECTIONS,
                    step = 1,
            ...
            item(key = "buffer_net_parallel_chunk_size") {
                ...
                val maxChunkSizeMb = if (playerSettings.playtorrioPerformanceModeEnabled) {
