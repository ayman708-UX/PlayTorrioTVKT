...
import com.playtorrio.tv.data.local.SubtitleOrganizationMode
import com.playtorrio.tv.data.local.TrailerSettings
...
class PlaybackSettingsViewModel @Inject constructor(
    ...
    suspend fun setBufferTargetSizeMb(mb: Int) {
        ...
        if (!current.useParallelConnections || current.playtorrioPerformanceModeEnabled || current.allowLargeTargetBuffer) {
    ...
    suspend fun resetBufferSettingsToDefaults() {
        ...
        val current = playerSettings.first()
        if (!current.useParallelConnections || current.playtorrioPerformanceModeEnabled || current.allowLargeTargetBuffer) return
    ...
    suspend fun setUseParallelConnections(enabled: Boolean) {
        ...
        val current = playerSettings.first()
        if (current.playtorrioPerformanceModeEnabled || current.allowLargeTargetBuffer) {
    ...
    suspend fun setPlayTorrioPerformanceModeEnabled(enabled: Boolean) {
        playerSettingsDataStore.setPlayTorrioPerformanceModeEnabled(enabled)
    }
}
